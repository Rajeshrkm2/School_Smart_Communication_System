...
REQUIRED_USER_COLS = ["username", "password", "role", "name"]
...
def ensure_base_files():
    if not os.path.exists(USERS_FILE):
        default_users = pd.DataFrame([
            {"username": "admin", "password": "admin123", "role": "admin", "name": "Main Admin"},
            {"username": "teacher1", "password": "teach123", "role": "teacher", "name": "Sunitha"},
        ])
...
def login_user(username, password, users_df):
    match = users_df[
...
if not st.session_state.logged_in:
    ...
    if st.button("Login"):
        user = login_user(username, password, users_df)
        if user:
...
# Auto attendance sync once per login
...
if os.path.exists(MESSAGE_LOG_FILE):
    try:
        ...
        show_cols = [c for c in msg_logs.columns if c != "mobile_number"]
        st.dataframe(msg_logs[show_cols], width="stretch")
    except Exception as e:
